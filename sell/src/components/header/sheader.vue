<template>
	<div class="header">
		...
		<div class="content-container">
			...
			<!--支持的数目-->
			<div class="supports-count" @click="startShow">
				<span>{{seller.supports.length}}个</span><i class="icon-keyboard_arrow_right"></i>
			</div>
		</div>
		<!--公告-->
		<div class="bulletin" @click="startShow">
		...
		<!--调用商家评分组件-->
		<sstar v-show="star" v-if="seller" v-bind:seller="seller"></sstar>
	</div>
</template>
...
import sstar from "@/components/star/sstar";
...
.header{
	background-color: rgba(7,17,27,.5);/*透明背景色*/
	position: relative;
	.content-container{
		position: relative;
