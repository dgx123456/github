<template>
	<div class="goods">
			...
			<ul class="item-container">
				...
				<li v-for="(item,index) in goods" v-bind:class="{'menu-item':true,'active':index==currentIndex,'v-menu-item':true}" @click=clickIndex(index)>
					...
					<span class="name"> <!--设置内容垂直居中的字标签代码：display：table-cell； vertical-align：middle-->
						<!--根据数据的type值，显示不同的图标-->
						<span v-if="item.type>=0" class="icon" @click="classGroup" v-bind:class="classGroup[item.type]"></span> 
              {{item.name}}<!-- 显示当前项目的名称-->
					</span>
				</li>
			</ul>
		...
		<div class="goods-content" ref="gscroll">
			...
			<ul>
				<li v-for="good in goods" class="good v-good">
					<h1>{{good.name}}</h1>
					<ol>
						<li v-for="food in good.foods" class="food" @click="Eselectfood">
							<div class="avatar">
								<img v-bind:src="food.icon" alt="这是一张图片" />
							</div>
		...
		<shopcart v-bind:selectfoods="selectfoods" ref="shopcart" v-bind:seller="seller"></shopcart>
		<carlist v-bind:selectfoods="selectfoods" v-show="carlistshow" ref="carlist" ></carlist>
		<food v-bind:selectfood="selectfood" v-show="fooddetailshow" ref="food"></food>
	</div>
</template>
...
import food from "@/components/food/food";
	...
	export default {
		...
		methods: {
			...
			clickIndex(index){//点的是第几个已经知道了---获取右侧所有的li
				var lists = this.$refs.gscroll.getElementsByClassName("v-good");
				this.gscroll.scrollToElement(lists[index],300)
			},
			getball(target){
				console.log(target);
				this.$refs.shopcart.ballchange(target);//使用子链 获取子标签，使用子组件调用子组件的方法
			}
		},
		//获取数据  及调用函数
		created: function() {
